...
#     rows = c.fetchall()
#     tasks = [{"id": r[0], "title": r[1], "desc": r[2], "completed": r[3]} for r in rows]
#     return tasks

# def add_task(title, desc):
#     c.execute("INSERT INTO tasks (title, desc, completed) VALUES (?, ?, ?)", (title, desc, False))
#     conn.commit()

# def update_task(id, title, desc):
#     c.execute("UPDATE tasks SET title=?, desc=? WHERE id=?", (title, desc, id))
#     conn.commit()
...
# # Custom CSS (Fix background + styling)
# st.markdown("""
#     <style>
...
# # Heading
# st.markdown(
#     '<div style="color:#00FF00; font-size:60px; font-weight:bold; text-align:center; margin-bottom:30px;">ANDROID CLUB</div>', 
#     unsafe_allow_html=True
# )

# # Add Task Heading
# st.markdown(
#     '<div style="color:#00BFFF; font-size:30px; font-weight:bold; margin-bottom:20px;">➕ Add a New Task</div>', 
#     unsafe_allow_html=True
# )
...
#             st.rerun()  # refresh immediately so new task shows
#         else:
...
#     with st.container():
#         st.markdown(
#             f"<div class='task-box'><b>{task['title']}</b><br>{task['desc']}</div>",
#             unsafe_allow_html=True
#         )
...
#                 key=f"chk_{task['id']}",
#                 on_change=lambda id=task["id"], completed=task["completed"]: toggle_task(id, completed)
#             )
#         with cols[1]:
...
#     c.execute("SELECT * FROM tasks")
#     rows = c.fetchall()
#     tasks = [{"id": r[0], "title": r[1], "description": r[2], "completed": r[3]} for r in rows]
#     return tasks
...
#             st.success("✅ Task added successfully!")
#             st.session_state['refresh'] = True  # Trigger UI refresh
#         else:
#             st.warning("⚠️ Please enter a title before adding.")
...
#         st.markdown(
#             f"<div class='task-box'><b style='color:{title_color};'>{task['title']}</b><br>{task['description']}</div>",
#             unsafe_allow_html=True
#         )
#         cols = st.columns([1,1,1])
#         with cols[0]:
...
#     st.session_state['refresh'] = False
#     st.experimental_rerun()  # This should work in most versions; if not, just re-run the script manually
...
# form .stButton>button { background-color:black !important; color:pink !important; border:2px solid pink !important; border-radius:8px; font-weight:bold; padding:6px 20px; transition:0.3s; }
# form .stButton>button:hover { background-color:black !important; color:pink !important; box-shadow:0px 0px 15px pink !important; }
...
# # ------------------ Header ------------------
# st.markdown('<div style="color:#00FF00; font-size:60px; font-weight:bold; text-align:center; margin-bottom:30px;">ANDROID CLUB</div>', unsafe_allow_html=True)
...
# if st.session_state.refresh:
#     st.session_state.refresh = False
#     st.experimental_rerun = lambda: None  # Dummy function, prevents AttributeError
#     # UI will update naturally without calling experimental_rerun
